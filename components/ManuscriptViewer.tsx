...
export const ManuscriptViewer: React.FC<ManuscriptViewerProps> = ({ pdf, lang }) => {
  ...
  // نظام تتبع وقت القراءة التراكمي باستخدام localStorage
  useEffect(() => {
    if (!pdf.name) return;
    
    // استرجاع الوقت القديم للملف الحالي
    const storageKey = `reading_time_${pdf.name}`;
    ...
    const timer = setInterval(() => {
      setReadingTime(prev => {
        ...
        // حفظ الوقت الجديد في كل ثانية لضمان التراكم
      ...
      <div className="h-14 bg-black/95 border-b border-white/10 flex flex-col z-30 shrink-0">
        <div className="flex items-center justify-between px-3 h-8 border-b border-white/5">
          ...
          <div className="flex items-center gap-2">
             ...
             <form onSubmit={goToPage} className="flex items-center bg-white/5 rounded-md border border-white/10 px-1 h-5">
                <input 
                  type="text" 
                  value={jumpPage}
                  onChange={(e) => setJumpPage(e.target.value)}
                  placeholder={lang === 'ar' ? 'اذهب...' : 'Go...'}
                  className="bg-transparent text-[8px] text-white w-8 outline-none text-center"
                />
