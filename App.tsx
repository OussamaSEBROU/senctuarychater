...
const App: React.FC = () => {
  ...
  const handleSynthesis = async (base64: string) => {
    ...
    setError(null);
    setAxioms([]); // Reset axioms
    setFlowStep('axioms'); // Start with axioms view
    ...
    try {
      ...
      if (extracted && extracted.length > 0) {
        ...
        // Fallback or error if no axioms extracted
        setError(lang === 'ar' ? "لم يتم استخراج بديهيات." : "No axioms extracted.");
      }
    } catch (err) {
      console.error(err);
    ...
    <div className={`fixed inset-0 flex flex-col bg-[#020202] text-white ${lang === 'ar' ? 'rtl' : 'ltr'} overflow-hidden transition-colors duration-500`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className="h-14 md:h-20 px-4 md:px-10 flex items-center justify-between border-b border-white/5 bg-black/40 backdrop-blur-3xl z-[60] shrink-0">
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick={() => setIsSidebarOpen(true)} className="p-2 md:p-3 hover:bg-white/5 rounded-xl transition-all border border-transparent hover:border-white/10">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <h1 className="hidden md:block text-[10px] font-black tracking-[0.6em] text-glow uppercase ml-2 text-white/80">{t.title}</h1>
        </div>
        ...
        <div className="md:absolute md:left-1/2 md:-translate-x-1/2 flex items-center gap-2 md:gap-3">
           ...
           <h2 className="text-[9px] md:text-[10px] font-black tracking-[0.2em] md:tracking-[0.3em] text-white/60 uppercase truncate max-w-[120px] md:max-w-none">{pdf ? pdf.name : t.sanctuary}</h2>
        </div>
        ...
        <div className="flex items-center gap-3">
           {pdf && (
             <button 
               ...
               className={`p-2.5 md:p-3 rounded-xl transition-all flex items-center gap-2 border ${showViewer ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/5 text-white/40 hover:text-white'}`}
             >
      ...
      <main className="flex-1 overflow-hidden relative z-10">
        {!pdf ? (
          <div className="h-full flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-1000">
            <h2 className="text-5xl md:text-9xl font-black sanctuary-3d mb-4 leading-none select-none">{t.sanctuary}</h2>
            <p className="mb-12 text-[8px] md:text-xs font-black tracking-[0.3em] md:tracking-[0.4em] uppercase text-white/30 text-glow-indigo-400">{t.introText}</p>
            <label className="w-full max-w-sm group relative block aspect-[1.4/1] border border-dashed border-white/10 rounded-[2.5rem] md:rounded-[4rem] hover:border-indigo-500/40 transition-all cursor-pointer bg-white/[0.01] hover:bg-white/[0.03] shadow-inner-xl">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 md:space-y-6">
                 <div className="w-16 h-16 md:w-20 md:h-20 rounded-[2rem] md:rounded-[2.5rem] bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-indigo-500/10 group-hover:scale-110 transition-all duration-500">
                    <svg className="w-8 h-8 md:w-10 md:h-10 text-white/10 group-hover:text-indigo-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                 </div>
                 <span className="text-[9px] md:text-[10px] font-black tracking-[0.4em] text-white/20 uppercase group-hover:text-white/50 transition-colors">{t.upload}</span>
              </div>
            </label>
              ...
              <div className="h-full flex flex-col items-center justify-center p-4 md:p-8 overflow-hidden animate-in fade-in zoom-in-95 duration-700">
                <div className="max-w-7xl mx-auto w-full flex flex-col gap-6 md:gap-12">
                   <div className="text-center px-4">
                      ...
                      <p className="text-white/30 font-serif italic text-xs md:text-xl opacity-60">Deep structural patterns identified within the manuscript</p>
                   </div>
                   ...
                   <div className="relative w-full">
                     ...
                     >
                        {axioms.length > 0 ? axioms.map((axiom, i) => (
                          ...
                          <div className="w-full text-center py-20 opacity-20 uppercase font-black tracking-widest">
                            No Axioms Extracted
                          </div>
                        )}
                     </div>
              ...
              <div className="h-full flex gap-0 md:gap-8 p-0 md:p-10 animate-in slide-in-from-bottom-12 duration-700 relative overflow-hidden">
                 ...
                 <div className={`flex-1 h-full flex flex-col max-w-full lg:max-w-5xl mx-auto w-full relative z-10`}>
                    <div className="hidden md:flex mb-6 justify-between items-center px-8">
                       <button onClick={() => setFlowStep('axioms')} className="text-[10px] font-black tracking-[0.5em] text-white/20 hover:text-white transition-colors flex items-center gap-3 group uppercase">
