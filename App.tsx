      ...
      {isSynthesizing && (
        <div className="fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
          <div className="relative">
            <div className="spinner-arc w-20 h-20 border-t-orange-600"></div>
        ...
        {pdf && (
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all border text-[10px] font-black tracking-widest uppercase ${showViewer ? 'bg-orange-600 border-orange-500 text-white shadow-[0_0_20px_rgba(163,74,40,0.4)]' : 'bg-white/5 border-white/5 text-white/40 hover:text-white'}`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
              <span className="hidden md:inline">{showViewer ? (lang === 'ar' ? 'إغلاق العارض' : 'Close Theater') : (lang === 'ar' ? 'فتح المخطوط' : 'Open Theater')}</span>
            </button>
          </div>
        )}
      </header>
      ...
      <main className="flex-1 overflow-hidden relative z-10">
        {!pdf ? (
          ...
          <div className="h-full flex flex-col lg:flex-row relative">
            {/* The Reader / Chat Split */}
            <div className={`flex-1 flex flex-col transition-all duration-700 ease-in-out ${showViewer ? 'lg:w-[40%] opacity-100' : 'lg:w-full'}`}>
              {flowStep === 'axioms' && (
            ...
            {/* Dedicated Research Viewer Pane - Archive Style */}
            {showViewer && (
              <div className={`fixed inset-0 lg:relative lg:inset-auto lg:w-[60%] bg-black z-[70] lg:z-10 animate-in slide-in-from-right duration-500 border-l border-white/10 flex flex-col shadow-[-20px_0_50px_rgba(0,0,0,0.8)]`}>
