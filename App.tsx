      ...
      <main className="flex-1 overflow-hidden relative z-10">
        {!pdf ? (
          <div className="h-full flex flex-col items-center justify-center p-6 text-center">
            {/* Removed sanctuary-3d and text-glow to satisfy 'remove radiation' request */}
            <h2 className="text-6xl md:text-9xl font-black mb-4 select-none text-white tracking-tight">{t.sanctuary}</h2>
            {/* Intro text colored in dark burnt orange (#a34a28) */}
            <p className="mb-12 text-sm md:text-base font-extrabold tracking-wide uppercase text-[#a34a28] max-w-2xl">
              {t.introText}
            </p>
            <label className="w-full max-w-sm group relative block aspect-[1.4/1] border border-dashed border-white/10 rounded-[3rem] hover:border-[#a34a28]/40 transition-all cursor-pointer bg-white/[0.01]">
          ...
          <div className="h-full flex flex-col">
            {flowStep === 'axioms' && (
              <div className="h-full flex flex-col items-center justify-center p-4">
                 <h3 className="text-2xl md:text-5xl font-black mb-8 uppercase text-center text-white/90">{t.axiomsTitle}</h3>
                 ...
                 <button onClick={() => setFlowStep('chat')} className="px-12 py-5 bg-[#a34a28] rounded-full font-black text-xs tracking-widest uppercase hover:bg-orange-800 transition-all shadow-lg shadow-orange-950/20">
                   {t.deepChatBtn}
                 </button>
              </div>
            )}
            {flowStep === 'chat' && (
              <div className="h-full flex relative overflow-hidden">
                {/* PDF Viewer as Overlay for both desktop and mobile */}
                {showViewer && (
                  <div className="absolute inset-0 lg:relative lg:w-1/2 bg-black z-[70] lg:z-10 animate-in fade-in slide-in-from-right duration-300">
                    <div className="absolute top-4 right-4 z-[80] flex gap-2">
                       <button onClick={() => setShowViewer(false)} className="bg-white/10 backdrop-blur-md p-2 rounded-full border border-white/10 hover:bg-white/20 transition-all">
                         <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth="2" strokeLinecap="round"/></svg>
                       </button>
                    </div>
                    <ManuscriptViewer pdf={pdf} lang={lang} />
                  </div>
                )}
                <div className={`flex-1 h-full bg-[#080808] transition-all duration-500 ${showViewer ? 'lg:w-1/2' : 'w-full'}`}>
